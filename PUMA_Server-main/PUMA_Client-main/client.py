...
def run_tray():
    ...
    try:
        ...
        print("Failed to load tray icon:", e)
        return
...
def save_report_on_server(request: ReportRequest):
    ...
    try:
        ...
        # 发送POST请求到文件处理服务
        response = requests.post(request.url, data={"uuid": request.uuid}, files=files_to_upload, verify=False)
...
async def get_project_info(uuid: str):
    ...
    try:
        ...
        project_profile = pcminfo_pb2.ProjectProfile()
